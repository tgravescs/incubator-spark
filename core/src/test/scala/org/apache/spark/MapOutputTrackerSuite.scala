...
class MapOutputTrackerSuite extends FunSuite with LocalSparkContext {
  ...
  test("remote fetch") {
    ...
    val (actorSystem, boundPort) = AkkaUtils.createActorSystem("spark", hostname, 
      0, new SecurityManager())
    ...
    val (slaveSystem, _) = AkkaUtils.createActorSystem("spark-slave", hostname, 
      0, new SecurityManager())
