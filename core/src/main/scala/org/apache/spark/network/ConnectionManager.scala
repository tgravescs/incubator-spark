...
private[spark] class ConnectionManager(port: Int) extends Logging {
        ...
        try {
          ...
          writeRunnableStarted.synchronized {
            ...
            val needReregister = register || conn.resetForceReregister() || 
              conn.resetForceReregisterSecurity()
  ...
  private def handleClientAuthNeg(
    ...
    if (waitingConn.sparkSaslClient.isComplete()) {
      logDebug("Client sasl completed for id: "  + waitingConn.connectionId)
      // make connection established and remove from connectionsAwaitning Sasl 
      // before the checkRegister call
      waitingConn.saslContextEstablished.set(true)
      connectionsAwaitingSasl -= waitingConn.connectionId
      if (waitingConn.checkRegister()) waitingConn.registerInterest()
      return
    } else {
  ...
  private def handleServerAuthNeg(
      ...
      securityMsg: SecurityMessage,
      connectionId: ConnectionId) {
    if (!connection.saslContextEstablished.get()) { 
      ...
      try {
        ...
        if (connection.sparkSaslServer.isComplete()) {
          logDebug("Server sasl completed: " + connection.connectionId) 
          connection.saslContextEstablished.set(true)
        } else {
  ...
  private def handleAuthentication(conn: Connection, bufferMessage: BufferMessage): Boolean = {
    logDebug("HandleAuthentication security is on")
    if (bufferMessage.isSecurityNeg) {
      ...
      val waitingConn = connectionsAwaitingSasl.getOrElse(connectionId, null)
      ...
      if (!conn.saslContextEstablished.get()) { 
      ...
      conn.synchronized {
        if (conn.sparkSaslClient == null) {
          ...
          try {
            ...
            case e: Exception => {
              logError("Error getting first response from the SaslClient")
              conn.close()
  ...
  private def sendSecurityMessage(connManagerId: ConnectionManagerId, message: Message) {
    ...
    connection.sendSecurity(message)

    wakeupSelector()
  }

  // Another option is to perhaps use sync, wait, notify and make the senders block while we do 
  // security negotiation.  That should simplify the logic in Connection dealing with multiple 
  // outboxes
